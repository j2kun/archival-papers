...
%\usepackage{fullpage} %standard small margins
...
%define a command with 1 parameter
...
%Complex networks are frequently used to model natural, social, and engineered
%systems.  A lot of studies looked at the problem of modeling the topology of a
%network or designing it.  Less is known, even in a stylized form, about how one
%might optimally install a given network topology, assuming that installing a
%node aids the installation of neighboring nodes.  This problem arises in
%settings such as engineering whenever damaged infrastructure networks are to be
%recovered.  In this paper we study the Neighbor Aided Network Installation
%Problem (NANIP) model of network recovery. We show that the problem is NP-hard
%even when restricted to a subclass of cost functions of practical interest,
%give a new integer programming formulation which is more efficiently solvable
%in practice, and prove a lower bound for the performance of the greedy
%algorithm.
...
%\blfootnote{$^{1}$ University of Illinois at Chicago \href{us@uic.edu}{us@uic.edu}.}
...
%Another algorithm achieves some good approximation...
...
together we have the following convex integer program: %given in Figure~\ref{fig:ip}. 
...
\end{align*}
%\caption{The convex integer programming formulation of convex NANIP \label{fig:ip}}
\end{centering}
\end{figure}
...
%Talk about the integrality gap...

%NANIP with a decreasing convex cost $f$ can be formulated as an integer program
%in the following way.  For each edge $(i,j)$ (considering direction) introduce
%a weight $E_{ij} \in \{0,1\}$ so that $E_{ij} + E_{ji} = 1$ for all $i,j\in V$.
%Through $E_{ij}$ variables we express $r(j,G,\sigma)$, the number of neighbors
%of a node $j$ installed before $j$, i.e., $r(j,G,\sigma) = \sum_{i\in
%N(j)}{E_{ij}}$, where $N(j)$ denotes the set of neighbors of $j$.  Lastly, let
%$l_d(x)$ define the downward sloping line through the points $(d-1,f(d-1))$ and
%$(d,f(d))$ with $d\in \{1,2,\dots,\}$.  Collectively those lines define a
%polyhedron whose extreme points coincide with the values of $f(d)$.  Thus
%$l_d(x) = f(d) + \left(f(d) - f(d-1)\right)(x-d)$.  To ensure that no two nodes
%are installed concurrently, for each $i\in V$, we introduce a variable $u_i \in
%[0,|V|]$ which depends on $E_{ij}$.  NANIP is the problem given by
%
%\begin{align}
%\min &\sum_{i=1}^{n} c_i.\label{eq:NANIP-IP}& \\
%\mbox {s.t. ~~~} & c_j \geq f(d) + \left(f(d) - f(d-1)\right)\left(\sum_{i\in N(j)}{E_{ij}}-d\right) & j\in V, d=1,2,\dots,n-1 \label{eq:NANIP-ld}\\
% & u_{i} - u_{j} + 1 \leq |V|(1-E_{ij}) \mbox{ and } u_i \in [0,|V|] & i\in V, j \in N(i)\notag\\
% & E_{ij} + E_{ji} = 1 \mbox{ and } E_{ij} \in \{0,1\} & i\in V, j \in N(i) \notag\\
%\end{align}
%
%Observe that because of the convexity of $f$, the points in the intersection of
%the inequalities $(\ref{eq:NANIP-ld})$ are the points above the convex hull of
%$\{f(d)\}_{d=1}^n$.  At an optimal $\sigma$, the value of $c_i$ would minimize
%$f(x)$ subject to $x\leq\sum_{i\in N(j)}{E_{ij}}$, so that the value of $c_i =
%f(r(i,G,\sigma))$ .
