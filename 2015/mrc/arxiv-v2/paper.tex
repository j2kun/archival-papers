...
whether to run the $r$-th map or reduce function, the total size of the input
...
$\langle 1, r, n \rangle$. In particular, if the number of rounds is $2^{\omega(n^c)}$ for all constants $c<1$, the space constraints would prohibit it from storing the round number, which prohibits any MRC algorithm from having this many rounds.
\end{remark}
