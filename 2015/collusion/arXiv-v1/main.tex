...
\keywords{Information monitoring, Routing networks, Border Gateway Protocol, Autonomous Systems, Route Hijacking, Traffic Interception}
...
%\footnotetext{Maps were taken from https://commons.wikimedia.org/wiki/Category:Blank\_maps\_of\_Europe#/media/File:BlankMap-Europe-v5.png
%and https://commons.wikimedia.org/wiki/Category:Blank\_maps\_of\_the\_United\_States#/media/File:BlankMap-USA.png}
\begin{itemize}
...
%with two variations regarding the
...
%In this section, we formally define our model by specifying the roles of the two types of agents.

%We then prove three propositions: that optimally picking agents to collude is NP-hard even ignoring lies, that the optimization problem associated with our model is not submodular, and that the nonuniform setting reduces to the uniform setting. 

%FK NOTE: we already said this above.
...
%    \end{cases}
%\]
%and updates with the synchronization protocol as follows.
%
%
%That is, at each step, $b_{i,j}^{t+1}$ updates to reflect the believed shortest path to $j$ from $j$. We call the path from $u$ to $v$ following the $b_{i,j}^{t}$ the {\it corresponding path}.
%Observe that, as with the $d_{i,j}^{t}$, if all agents are honest, the $b_{i,j}^{t}$ will converge in at most $n-1$ steps.
%
%Since the $argmin$ above may have several possibilities, it is necessary to provide a tie-breaking criterion. For ease of analysis, we use the following:
%\begin{enumerate}
%   \item If possible, an honest node set $b_{i,j}^t$ to a neighboring honest node such that the corresponding path avoids colluding nodes.
%   \item otherwise, the honest node set $b_{i,j}^t$ to a neighbor achieving the minimum in \ref{1} of the colluding agents' choice.
%\end{enumerate}
%
%This tie-breaking criterion provides a worst-case assumption for the colluding agents while still maintaining the ability for messages to be appropriately routed. %The criterion above is equivalent to sending messages to 

%The above rule is equivalent to chosing noted by~\cite{GoldbergSHR10} and others, the internal policies used by autonomous systems in BGP are private information. 

%
%We now describe the protocol followed by honest agents for updating their
%beliefs and forwarding messages in $G$, illustrated in figure \ref{fig:synchronization}. Each agent $i$ maintains two length-$n$
...
%In our collusion model we allow colluding agents to broadcast false distances
...
%to be sent back and forth in the network without reaching its destination (figure \ref{fig:admissibility}), so we restrict the space of admissible strategies to be those that do not deterministically cause such cycles. In the language of BGP, we disallow ``black holes.'' We call such strategies \emph{admissible}.
%\begin{figure}
%\centering
%\includegraphics[width=\textwidth]{images/Badmissibility.pdf}
%\caption{}
%\label{fig:admissibility}
%\end{figure}
%
%We introduce two different types of behaviors of colluding agents that give rise to two versions of our
%model. We call the first \emph{uniform broadcasts}, where an agent during synchronization protocol broadcasts same information to all of its neighbors. In notation, a \emph{broadcast policy} is a sequence of
...
%\subsection{Florentine nobility} \label{sec:nobility}We say a path from $s$ to $t$, $s= v_0, v_1, \ldots, v_{\ell} =  t$, is {\it corresponding path} if $v_{i+1} \in b_{v_i,t}$ for $i=0, \ldots, \ell$. That is, a corresponding path has each honest agent forward each message to a neighbor advertising a minimum distance.
%
...
the colluding agents must establish a more precise broadcast strategy, $\rho(v,t,i)$, for each colluding agent $v$ and each neighbor $i$. %In turn, each honest agent only considers the distances broasdcast to them in equations \ref{uniformd0}, \ref{uniformd} and \ref{uniformb}.
...
model as follows.  (See figure 4 for an illustration). For simplicity we will prove the case where $G$ is
...
\caption{An illustration for Proposition \ref{prop:uniform-reduction}. Given a graph $G$, a set of colluding agents $S$ (shaded circles), and a fraction $p$ for the nonuniform model (left panel), we produce a new graph $G'$, a new set of colluding agents $S'$ (shaded circles), and a fraction $p'$ for the uniform model (right panel) by inserting a new colluding agent to every edge connecting a colluding agent in $S$ and a honest node. We show in the proposition that a nonuniform broadcasting strategy in the left is equivalent to an uniform broadcasting strategy in the right.}
\label{fig:nonuniform-cycle}
\end{figure}
...
%The forwarding policy is as follows. 
...
%where agents can be adjacent. Shaded nodes are colluding. If the component with $z,y$ is processed first then our algorithm correctly chooses the $w,x$ component to have forwarding number 2 (with $x$ broadcasting 1), capturing all traffic from $a,b$ to $t$.  On the other hand if $w,x$ is processed first the result will miss traffic from $a,b$.} \label{fig:forwarding-ctex}} {\scalebox{1.5}{\includegraphics{images/forwarding-ctex.pdf}}} 
...
uniformly at random, while red curves to subsets chosen by largest
degrees. Dashed curves correspond to the strategy where each agent independently lies
by an additive factor of two, while solid curves to the optimal separated
strategy of Section~\ref{sec:separated}. The inset graph for the AS network
magnifies the leftmost portion of the blue curves.}
