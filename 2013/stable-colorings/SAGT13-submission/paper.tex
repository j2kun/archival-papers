...
%One preliminary problem we tackle is computing the price of anarchy for our game.
%This concept
...
%the best and worst-case Nash equilibria. Much work has since focused on this
%concept, e.g.~\cite{FKKMS02,RT02}.
% Computing 
%the price of anarchy for the game we present in this paper is much simpler than
%in their work, but an interesting question nonetheless.
...
% and to contrast
%with our hardness results for the strict and directed cases.  
...
%by giving an algorithm that
%finds a $k$-unfriendly partition for all $k$.
...
%BP: Matching pennies is just a game played by a coordinator and an anticoordinator, and is more of a classic example of a game than something from sociological research - but the fashion game is their invention I believe.
Cao and Yang~\cite{CaoY12a} also study
a related problem originating from sociology, called the
\emph{matching pennies game}, % or  \emph{fashion game}, 
where some
...
%This line of work differs from ours in that we do not try to
%preserve proper colorings and we seek to maximize a local payoff.
% instead of a
%global one.
...
%BP: should we say "generalizing the 'fashion game' to more than two strategies" instead?
...
%where the goal is to divide a graph into two parts maximizing the number of
